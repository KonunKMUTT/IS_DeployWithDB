...
# Streamlit app
def main():
    st.markdown('<p style="text-align:center; font-weight:bold; font-size:50px;">Heart Disease Prediction App</p>', unsafe_allow_html=True)
    st.markdown('<p style="text-align:center; color:orange; font-size:11px;">This program was developed as a master\'s degree research project under the DBIS Program of School of Information Technology,</p>',unsafe_allow_html=True)
    st.markdown('<p style="text-align:center; color:orange; font-size:11px;">King Mongkut\'s University of Technology Thonburi - KMUTT.</p>',unsafe_allow_html=True)
    st.warning('Disclaimer : The predictions generated by this system are solely for research purposes and should not be interpreted as medical advice. This system is not a substitute for professional medical judgment and should not be used to make medical decisions. It is important to seek consultation with a qualified healthcare professional for any medical concerns or questions.', icon="⚠️")
    st.markdown('<br><p style="text-align:center; font-weight:bold; font-size:20px">Enter the data to prediction, Please Ensure all feilds are filled.</p><br>', unsafe_allow_html=True)
